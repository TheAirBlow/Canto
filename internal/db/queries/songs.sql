...
SELECT * FROM songs WHERE name = $1 OR name_normalized = $1 LIMIT 1;

-- name: FindSongByExactNameForArtists :one
SELECT s.* FROM songs s
JOIN song_artists sa ON sa.song_id = s.id
WHERE (s.name = sqlc.arg(name)::text OR s.name_normalized = sqlc.arg(name)::text) AND sa.artist_id = ANY(sqlc.arg(artist_ids)::bigint[])
LIMIT 1;
...
WHERE (s.name = sqlc.arg(name)::text OR s.name_normalized = sqlc.arg(name)::text)
  AND sa.artist_id = ANY(sqlc.arg(artist_ids)::bigint[])
  AND sal.album_id = sqlc.arg(album_id)::bigint
LIMIT 1;

-- name: CreateSong :one
INSERT INTO songs (name, name_normalized, duration_ms) VALUES ($1, $2, $3) RETURNING *;

-- name: UpdateSong :one
UPDATE songs SET name = $2, name_normalized = $3, pinned = TRUE, updated_at = now() WHERE id = $1 RETURNING *;
